...
# ==================== WATER STATIONS ENDPOINTS ====================

# @app.get("/api/stations", response_model=List[WaterStationResponse])
...
#     - refresh: Force refresh from government API / CSV
    
#     Data source priority:
#     1. Government API (CPCB/data.gov.in) - if available
#     2. Local CSV file (data.csv) - fallback if API fails
#     3. Existing database records - if no new data
#     """
...
#         if gov_stations:
#             # Sync government API data to database
...
#     """Get total count of stations in database"""
...
# @app.get("/api/stations/{station_id}", response_model=WaterStationResponse)
# def get_station(station_id: int, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
#     station = db.query(WaterStation).filter(WaterStation.id == station_id).first()
#     if not station:
#         raise HTTPException(status_code=404, detail="Station not found")
#     return station

# # Create Station - Only Authority and Admin
# @app.post("/api/stations", response_model=WaterStationResponse)
# def create_station(
#     station: WaterStationCreate,
#     current_user: User = Depends(require_role(["admin", "authority"])),
...
# # Add Reading - NGO, Authority, Admin can add readings (NOT Citizen)
# @app.post("/api/stations/{station_id}/readings", response_model=StationReadingResponse)
# def create_reading(
#     station_id: int,
#     reading: StationReadingCreate,
#     current_user: User = Depends(require_role(["ngo", "authority", "admin"])),
#     db: Session = Depends(get_db)
# ):
#     station = db.query(WaterStation).filter(WaterStation.id == station_id).first()
#     if not station:
...
#             alert = Alert(
#                 message=f"pH Level out of range at {station.name}: {value}",
#                 severity="Critical"
#             )
#             db.add(alert)
    
#     if reading.parameter == "turbidity":
#         value = float(reading.value)
#         if value > 5.0:
#             alert = Alert(
#                 message=f"High Turbidity at {station.name}: {value} NTU",
#                 severity="Warning"
#             )
#             db.add(alert)
...
# @app.get("/api/search/history", response_model=List[SearchResponse])
...
# # Update Report Status - Authority and Admin only (NOT NGO, NOT Citizen)
...
#     if status not in [ReportStatus.PENDING.value, ReportStatus.VERIFIED.value, ReportStatus.REJECTED.value]:
...
#             latitude=17.3850,
#             longitude=78.4867,
#             managed_by="Telangana State Pollution Control Board"
#         ),
#         WaterStation(
#             name="Secunderabad Station",
#             location="Secunderabad Lake",
#             region="Telangana",
#             state="Telangana",
#             latitude=17.4399,
#             longitude=78.4983,
...
#         ),
#         WaterStation(
#             name="Gachibowli Monitoring",
#             location="Gachibowli Lake",
#             region="Telangana",
#             state="Telangana",
#             latitude=17.4439,
#             longitude=78.3510,
...
#             location="Marina Beach Water",
#             region="Tamil Nadu",
#             state="Tamil Nadu",
#             latitude=13.0827,
#             longitude=80.2707,
#             managed_by="Tamil Nadu Pollution Control Board"
#         ),
#         WaterStation(
#             name="Bangalore Monitoring",
#             location="Ulsoor Lake",
#             region="Karnataka",
#             state="Karnataka",
#             latitude=12.9716,
#             longitude=77.5946,
#             managed_by="Karnataka State Pollution Control Board"
#         )
#     ]
    
#     for station in stations:
#         existing = db.query(WaterStation).filter(WaterStation.name == station.name).first()
#         if not existing:
#             db.add(station)
...
# """
# Water Quality Monitor - FastAPI Backend
# Milestone 2: Real-Time Water Data & Maps (Dataset Integrated)
# """

# from fastapi import FastAPI, Depends, HTTPException, status, File, UploadFile, Query
# from fastapi.middleware.cors import CORSMiddleware
# from fastapi.security import HTTPBearer, HTTPAuthorizationCredentials
# from sqlalchemy import create_engine, Column, Integer, String, Float, DateTime, Text, Enum, ForeignKey, Numeric, Boolean
# from sqlalchemy.ext.declarative import declarative_base
# from sqlalchemy.orm import sessionmaker, Session, relationship
# from sqlalchemy.sql import func
# from pydantic import BaseModel, EmailStr
...
# # Database Configuration
# # Ensure you have created this database in PostgreSQL
# DATABASE_URL = "postgresql://postgres:3906DW82@localhost:5432/watermonitor"
...
# # Initialize FastAPI app
# app = FastAPI(title="Water Quality Monitor API", version="2.1.0")
...
#     __tablename__ = "water_stations"
#     id = Column(Integer, primary_key=True, index=True)
#     name = Column(String(100), nullable=False) # Maps to Station Code
#     location = Column(String(500), nullable=False)
#     region = Column(String(100))
#     state = Column(String(100))
#     country = Column(String(100), default="India")
#     latitude = Column(Numeric(10, 8), default=0.0)
#     longitude = Column(Numeric(11, 8), default=0.0)
#     managed_by = Column(String(100))
#     created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
#     readings = relationship("StationReading", back_populates="station", cascade="all, delete-orphan")
...
#     year = Column(Integer) # To store the year from the dataset
#     recorded_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
#     station = relationship("WaterStation", back_populates="readings")
...
# def calculate_station_wqi(readings: List[StationReading]):
#     """Calculate Indian Water Quality Index (IWQI)"""
#     params = {r.parameter: float(r.value) for r in readings}
#     if not params: return None
    
#     score = 100
#     # pH logic (Ideal 6.5 - 8.5)
#     if "pH" in params:
#         ph = params["pH"]
#         if ph < 6.5 or ph > 8.5: score -= 15
    
#     # DO logic (Ideally > 4 mg/l)
#     if "DO" in params and params["DO"] < 4: score -= 20
    
#     # BOD logic (Ideally < 3 mg/l)
#     if "BOD" in params and params["BOD"] > 3: score -= 20
    
#     # Coliform logic
#     if "fecal_coliform" in params and params["fecal_coliform"] > 500: score -= 15

#     return max(0, score)

# def get_station_status(wqi):
#     if wqi is None: return "Unknown"
...
# @app.post("/api/admin/upload-cpcb-dataset")
# async def upload_cpcb_dataset(
#     file: UploadFile = File(...),
#     db: Session = Depends(get_db),
#     current_user: User = Depends(require_role(["admin", "authority"]))
# ):
#     """Upload the CPCB CSV dataset to bulk-populate the system"""
...
#     # Parameter Mapping (matching your CSV columns)
#     param_map = {
#         "Temp": "temperature",
#         "D.O. (mg/l)": "DO",
#         "PH": "pH",
...
#     records_created = 0
#     # Pre-clean column names to handle the encoding issues like '痠hos/cm'
#     df.columns = [c.split('(')[0].strip() if 'CONDUCTIVITY' in c else c for c in df.columns]

#     for _, row in df.iterrows():
#         # 1. Handle the WaterStation
#         station_code = str(row.get('STATION CODE'))
#         if station_code == 'nan': continue
        
#         station = db.query(WaterStation).filter(WaterStation.name == station_code).first()
#         if not station:
#             station = WaterStation(
#                 name=station_code,
#                 location=str(row.get('LOCATIONS', 'Unknown')),
...
#         for csv_col, db_param in param_map.items():
#             # Handle potential cleaned column name for Conductivity
#             col_to_use = "CONDUCTIVITY" if csv_col == "CONDUCTIVITY" else csv_col
#             val = row.get(col_to_use)
            
#             # Skip if value is null or "NAN"
#             if pd.isna(val) or str(val).strip().upper() == "NAN":
#                 continue
                
#             try:
#                 # Remove any non-numeric markers like '<' or '>'
...
#     db.commit()
#     return {"status": "success", "total_readings_imported": records_created}
...
# ):
#     query = db.query(WaterStation)
#     if state: query = query.filter(WaterStation.state.ilike(f"%{state}%"))
#     if search: query = query.filter(WaterStation.name.ilike(f"%{search}%") | WaterStation.location.ilike(f"%{search}%"))
#     return query.all()

# @app.get("/api/stations/{station_id}/current")
# def get_station_current(station_id: int, db: Session = Depends(get_db)):
#     station = db.query(WaterStation).filter(WaterStation.id == station_id).first()
#     if not station: raise HTTPException(status_code=404)
    
#     # Get latest reading for each parameter
#     all_readings = db.query(StationReading).filter(StationReading.station_id == station_id).order_by(StationReading.recorded_at.desc()).limit(20).all()
...
# @app.post("/api/reports")
# def create_report(report: ReportCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
#     db_report = Report(user_id=user.id, **report.dict())
#     db.add(db_report)
#     db.commit()
#     return db_report
...
#     return db.query(Alert).order_by(Alert.timestamp.desc()).limit(10).all()

# # ==================== LEGACY COMPATIBILITY ====================

# @app.get("/get-sensor-data")
# def legacy_data(db: Session = Depends(get_db)):
#     # Returns latest readings for the dashboard
#     reading = db.query(StationReading).order_by(StationReading.recorded_at.desc()).first()
#     return {
#         "current": {
#             "ph": float(reading.value) if reading and reading.parameter == "pH" else 7.2,
#             "temp": 25.0,
#             "status": "Stable"
#         },
#         "history": []
...
#             alert = Alert(
#                 type=AlertType.CONTAMINATION.value,
#                 message=f"pH Level out of safe range at {station.name} (Value: {value}).",
#                 location=station.location
#             )
#             db.add(alert)
...
#     if reading.parameter == "temperature":
#         value = float(reading.value)
#         if value > 100.0:
#             alert = Alert(
#                 type=AlertType.BOIL_NOTICE.value,
#                 message=f"Extreme Temperature detected at {station.name}: {value}°C. Boil notice issued.",
#                 location=station.location
#             )
#             db.add(alert)
#         elif value > 35.0:
#             alert = Alert(
#                 type=AlertType.CONTAMINATION.value,
#                 message=f"High Temperature detected at {station.name}: {value}°C. Potential risk.",
#                 location=station.location
#             )
#             db.add(alert)

#     if reading.parameter == "e_coli":
#         value = float(reading.value)
#         if value > 0:
#             alert = Alert(
#                 type=AlertType.CONTAMINATION.value,
#                 message=f"E. coli detected at {station.name}: {value} CFU/100mL. Avoid consumption.",
#                 location=station.location
#             )
#             db.add(alert)
...
#     """Load station data from water_dataX.csv (real government water quality data)"""
#     stations = []
...
#                 for row in reader:
#                     # station_name = row.get('LOCATIONS', '').strip()
#                     # state = row.get('STATE', '').strip()
                    
#                     # # Create a unique station name
#                     # name = f"Station {row.get('STATION CODE', '')}"
#                     # if station_name:
#                     #     # Clean up the location name
#                     #     location = station_name.replace('NAN', '').strip()
#                     #     name = f"{location}"
#                     station_name = row.get('LOCATIONS', '').strip()
#                     state = row.get('STATE', '').strip()

#                     location = station_name.replace('NAN', '').strip() if station_name else f"Water Station {row.get('STATION CODE', '')}"
...
#                     stations.append({
#                         'name': name[:100],
#                         'location': location[:200] if station_name else f"Water Station {row.get('STATION CODE', '')}",
#                         'region': state,
#                         'state': state,
#                         'country': 'India',
#                         'latitude': lat,
#                         'longitude': lon,
#                         'managed_by': f"{state} Pollution Control Board" if state else 'CPCB',
#                         # Store water quality data as additional info
#                         'temp': row.get('Temp', ''),
#                         'do': row.get('D.O. (mg/l)', ''),
#                         'ph': row.get('PH', ''),
#                         'conductivity': row.get('CONDUCTIVITY (�mhos/cm)', ''),
#                         'bod': row.get('B.O.D. (mg/l)', ''),
#                         'nitrate': row.get('NITRATENAN N+ NITRITENANN (mg/l)', ''),
#                         'fecal_coliform': row.get('FECAL COLIFORM (MPN/100ml)', ''),
#                         'total_coliform': row.get('TOTAL COLIFORM (MPN/100ml)Mean', ''),
#                         'year': row.get('year', '')
#                     })
...
#             continue
#         except Exception as e:
#             print(f"Error loading CSV with {enc}: {e}")
#             break
    
#     return stations

# def get_state_coordinates(state_name):
#     """Return approximate coordinates for a state in India"""
#     state_coords = {
...
#     import random
#     base_coords = state_coords.get(state_name.upper(), (20.5937, 78.9629))  # Default to India center
#     lat = base_coords[0] + random.uniform(-0.5, 0.5)
#     lon = base_coords[1] + random.uniform(-0.5, 0.5)
#     return lat, lon

# async def fetch_cpcb_stations():
#     """
#     Fetch water quality monitoring stations from CPCB (Central Pollution Control Board) India API
#     Returns list of stations or None if API fails
#     """
#     try:
#         # CPCB India Water Quality Data Portal API
#         # Using the India Water Resources Information System (WRIS) or similar government API
#         base_url = "https://cpcbenvis.nic.in/water_quality_data.html"
        
#         # Note: CPCB doesn't have a direct public REST API for real-time data
#         # We'll try to fetch from available open data sources
        
#         # Alternative: Using data.gov.in API if available
#         data_gov_api = "https://data.gov.in/backend/dms/v1/data"
        
#         async with httpx.AsyncClient(timeout=30.0) as client:
#             # Try to fetch from available government water quality data sources
#             # This is a placeholder for the actual CPCB/government API integration
            
#             # For demonstration, we'll return None to trigger CSV fallback
...
# async def fetch_government_water_stations():
#     """
#     Fetch water quality stations from government APIs
#     Tries multiple sources: CPCB, data.gov.in, etc.
#     Returns list of stations or None if all APIs fail
#     """
#     # Try CPCB first
#     cpcb_data = await fetch_cpcb_stations()
#     if cpcb_data:
#         return cpcb_data
    
#     # Add more government API sources here as needed
    
#     return None

# def sync_stations_to_db(stations_data: list, db: Session):
#     """Sync fetched station data to database"""
#     created_count = 0
    
#     for station_data in stations_data:
#         # Check if station already exists
#         existing = db.query(WaterStation).filter(
#             WaterStation.name == station_data['name']
#         ).first()
...
class Alert(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    station_id = Column(Integer, ForeignKey("water_stations.id"), nullable=True) # Added for Task 2
    type = Column(String(50), nullable=False)
    message = Column(Text, nullable=False)
    location = Column(String(200))
    issued_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
...
async def create_reading(
    ...
    # Addition 1: Trigger Predictive Engine Automatically
    background_tasks.add_task(run_predictive_engine, station_id)
    
    # Task 2 Deduplication wrapper for manual threshold alerts
    async def log_manual_alert(alert_type, message):
        await create_alert_with_dedupe(
            db=db,
            station_id=station_id,
    ...
    # Check for threshold alerts (Milestone 4 Logic)
    if reading.parameter == "pH":
        value = float(reading.value)
        if value < 6.5 or value > 8.5:
            await log_manual_alert(AlertType.CONTAMINATION.value, f"pH Level out of safe range at {station.name} (Value: {value}).")
    
    if reading.parameter == "turbidity":
        value = float(reading.value)
        if value > 5.0:
            await log_manual_alert(AlertType.CONTAMINATION.value, f"High Turbidity detected at {station.name}: {value} NTU")
            
    if reading.parameter == "temperature":
        value = float(reading.value)
        if value > 100.0:
            await log_manual_alert(AlertType.BOIL_NOTICE.value, f"Extreme Temperature detected at {station.name}: {value}°C. Boil notice issued.")
        elif value > 35.0:
            await log_manual_alert(AlertType.CONTAMINATION.value, f"High Temperature detected at {station.name}: {value}°C. Potential risk.")

    if reading.parameter == "e_coli":
        value = float(reading.value)
        if value > 0:
            await log_manual_alert(AlertType.CONTAMINATION.value, f"E. coli detected at {station.name}: {value} CFU/100mL. Avoid consumption.")
...
#         # Fetch DB alerts (India) unless country is explicitly 'USA'
...
#         # Fetch USGS alerts (USA) unless country is explicitly 'India'
#         if not country or country.upper() != 'INDIA':
#             usgs_alerts = await fetch_usgs_alerts()
#             for ua in usgs_alerts:
#                 if type and ua["type"] != type:
#                     continue
#                 ua["country"] = "USA"
#                 results.append(ua)
...
#         results.sort(key=lambda x: parse_date(x.get('issued_at')), reverse=True)
...
# WHO Thresholds for water quality parameters
WHO_THRESHOLDS = {
    ...
    "conductivity": {"max": 1500.0, "unit": "µS/cm"}
}

def analyse_station(station_id: int, db: Session):
    """Analyse a station's recent readings for predictive alerts.
    Rules:
      1. APPROACHING: 7-day rolling avg > 80% of WHO threshold
      2. BREACH: last 3 consecutive readings exceed threshold
    """
    alerts = []
    ...
    for param, thresholds in WHO_THRESHOLDS.items():
        readings = db.query(StationReading).filter(
            StationReading.station_id == station_id,
            StationReading.parameter == param,
            StationReading.recorded_at >= seven_days_ago
        ...
        # Check BREACH rule: last 3 consecutive readings exceed threshold
        breach = False
        approaching = False
        
        if "max" in thresholds:
            threshold = thresholds["max"]
            if all(v > threshold for v in last_3):
                breach = True
            elif avg_val > threshold * 0.8:
                approaching = True
        
        if "min" in thresholds and not breach:
            threshold = thresholds["min"]
            if all(v < threshold for v in last_3):
                breach = True
            elif avg_val < threshold * 1.2:
                approaching = True
        ...
        if breach:
            alerts.append({
                ...
                "threshold": thresholds.get("max", thresholds.get("min")),
                "alert_message": f"BREACH: {param} at {station.name} — last 3 readings exceed WHO threshold. Avg: {round(avg_val, 3)} {thresholds['unit']}",
                "location": station.location
            })
        elif approaching:
            alerts.append({
                ...
                "current_avg": round(avg_val, 3),
                "threshold": thresholds.get("max", thresholds.get("min")),
                "alert_message": f"APPROACHING: {param} at {station.name} — 7-day avg approaching WHO threshold. Avg: {round(avg_val, 3)} {thresholds['unit']}",
                "location": station.location
            })
...
class ConnectionManager:
    """Manages WebSocket connections for live alert push"""
    def __init__(self):
...
# Helper for Addition 3: Deduplication Logic
async def create_alert_with_dedupe(db: Session, station_id: Optional[int], alert_type: str, message: str, location: str, source: str = "manual"):
    """Correct Deduplication Logic as per Milestone 4 requirements"""
    ...
    query = db.query(Alert).filter(
        ...
        Alert.source == source,
        Alert.issued_at > twenty_four_hours_ago
    )
    
    # Only filter by station_id if provided (for manual alerts without station_id, skip this filter)
    ...
    if not existing:
        ...
        await ws_manager.broadcast({
            ...
            "payload": {
                ...
                "issued_at": new_alert.issued_at.isoformat() if new_alert.issued_at else None,
...
# Predict execution wrapper for background tasks
def run_predictive_engine(station_id: int):
    """Wrapper to run analysis in background with a fresh session"""
    db = SessionLocal()
    try:
        analyse_station(station_id, db)
    finally:
        db.close()

@app.post("/api/v1/alerts/predictive/generate")
...
# ==================== AGGREGATE READINGS ENDPOINT ====================

@app.get("/api/v1/stations/readings/aggregate")
...
# ==================== COLLABORATION & ADMIN ENDPOINTS ====================

@app.post("/api/v1/collaborations", response_model=CollaborationOut, status_code=201)
...
@app.get("/api/v1/collaborations", response_model=List[CollaborationOut])
...
# ==================== AUTO PREDICTIVE ALERTS ENDPOINT ====================
